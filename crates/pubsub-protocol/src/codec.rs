...
impl Decoder for PubSubCodec {
    ...
    fn decode(&mut self, src: &mut BytesMut) -> Result<Option<Frame>, PubSubError> {
        ...
        if body_len > self.max_frame_size {
            return Err(PubSubError::FrameTooLarge {
                size: body_len,
        ...
        if version != PROTOCOL_VERSION {
            ...
            return Err(PubSubError::UnsupportedVersion { version });
        }
        src.advance(1);
        ...
        let frame = rmp_serde::from_slice(msgpack_data)
            .map_err(|e| PubSubError::Codec(e.to_string()))?;
...
impl Encoder<Frame> for PubSubCodec {
    type Error = PubSubError;

    fn encode(&mut self, item: Frame, dst: &mut BytesMut) -> Result<(), PubSubError> {
        let msgpack_data =
            rmp_serde::to_vec(&item).map_err(|e| PubSubError::Codec(e.to_string()))?;
...
mod tests {
    ...
    fn codec() -> PubSubCodec {
        PubSubCodec::new(64 * 1024)
    }
    ...
    fn oversized_frame_rejected_on_decode() {
        ...
        let mut small_codec = PubSubCodec::new(10);
    ...
    fn version_mismatch_rejected() {
        ...
        assert!(matches!(
            result,
            Err(PubSubError::UnsupportedVersion { version: 99 })
        ));
    }
